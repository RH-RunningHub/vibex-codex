...
- On `SOURCE_REVISION_ID_CONFLICT`, the sealed tree hashed to an already-known revision (undo, revert to history, or oversized files stripped). This is not `SOURCE_CHANGED`. Do not replay the same commit or start a replacement session with the same plan. Fetch the project: if `source_state=CLEAN`, keep that stable revision and continue to `$vibex-preview`; tell the user no new revision was created because the tree already exists. If the session is still `EDITING` or `QUIESCING`, cancel the edit, fetch again, and only report a stuck session if it remains unclean. A true cross-project id collision is also terminal: stop and report, never loop.
- If initialize, commit, or operation data includes `flagged`, treat it as advisory metadata, not a failed save. Those paths were kept but will not be exported (`redacted` or `omitted`). Show each path and action to the user and keep the returned stable revision.
